...
/**
 * SCAN_MODE controls what happens when the Python OCR + VLM pipeline is not
 * available on this machine.
 *   live     (default) - always use the Python pipeline, fail loudly if absent
 *   fallback           - try Python first, fall back to the deterministic
 *                        rule engine in utils/ruleEngine.js when it fails
 *   demo               - skip Python entirely and use the rule engine
 *
 * The fallback path uses the same rule engine, the same JSON contract and the
 * same PDF generation, so nothing downstream can tell the difference apart
 * from the `engine` field on the response.
 */
const SCAN_MODE = (process.env.SCAN_MODE || 'live').toLowerCase();
...
function runLocalEngine(file, body, meta) {
  let extractedFields = null;
  if (body.extractedFields) {
    try {
      const parsed = JSON.parse(body.extractedFields);
      extractedFields = parsed.extractedFields || null;
    } catch (e) {
      extractedFields = null;
    }
  }
  const fields = extractedFields || demoExtraction(file.originalname || '');
  return checkCompliance(fields, meta, body.category || null);
}

router.post(
  '/scan-label',
  rateLimit({ windowMs: 60000, max: 20, message: 'Too many scans submitted. Please wait a minute.' }),
  upload.single('image'),
  async (req, res) => {
    try {
      if (!req.file) {
      ...
      let result;
      let engine = 'python-ocr-vlm';

      if (SCAN_MODE === 'demo') {
        result = runLocalEngine(req.file, body, meta);
        engine = 'local-rule-engine';
      } else {
        try {
          const parsedReport = await runPythonCompliance(req.file.path);
          result = adaptPythonReportToScanResult(parsedReport);
        } catch (e) {
          console.error('Python bridge error:', e.message);
          if (SCAN_MODE === 'fallback') {
            result = runLocalEngine(req.file, body, meta);
            engine = 'local-rule-engine-fallback';
          } else {
            return res.status(500).json({
              error: 'Internal pipeline error during compliance analysis.',
              hint: 'Set SCAN_MODE=fallback in server/.env to use the deterministic rule engine when the Python pipeline is unavailable.'
            });
          }
        }
      }
      ...
      try {
        ...
        // History is additive: a failure here must never break the scan itself.
        console.error('Inspection history write failed:', inspectionError.message);
      }
      ...
      res.status(201).json({
        ...
        productId: inspection ? inspection.productId : null,
        priority: inspection ? inspection.priority : null,
        priorityReason: inspection ? inspection.priorityReason : null
      });
    } catch (err) {
...
router.get('/scan-label/:id', (req, res) => {
  ...
  const hydrated = hydrateScan(row);
  if (row.inspectionId) {
    const inspection = getInspection(row.inspectionId);
    if (inspection) {
      hydrated.inspectionId = inspection.id;
      hydrated.productId = inspection.productId;
      hydrated.priority = inspection.priority;
      hydrated.priorityReason = inspection.priorityReason;
      hydrated.inspectionStatus = inspection.status;
      hydrated.findings = inspection.findings;
    }
  }
  res.json(hydrated);
});
