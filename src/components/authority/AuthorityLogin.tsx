...
import { Lock, ShieldAlert, Loader2, UserPlus } from 'lucide-react';
...
export const AuthorityLogin: React.FC<AuthorityLoginProps> = ({ onLoginSuccess, onBack }) => {
  ...
  return <AuthorityLoginForm onLoginSuccess={onLoginSuccess} onBack={onBack} onCreateAccount={() => setMode('register')} />;
};

const AuthorityLoginForm: React.FC<AuthorityLoginProps & { onCreateAccount: () => void }> = ({
  onLoginSuccess,
  onBack,
  onCreateAccount
}) => {
    ...
    <div className="max-w-md mx-auto mt-10 bg-[#EEF2F8] border border-[#D6DEEA] rounded-xl shadow-sm p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-1.5 text-xs font-semibold text-[#14224A] hover:text-[#B45309] underline text-center pt-1"
        >
          <UserPlus className="w-3.5 h-3.5" />
          New Authority? Create Authority Account
        </button>
