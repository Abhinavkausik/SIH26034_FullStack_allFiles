...
import { FeedbackForm, ProductFeedbackPanel } from './FeedbackPanel';
...
export const ConsumerScanner: React.FC<ConsumerScannerProps> = ({ onOpenRulebookWithClause, onOpenDocument }) => {
  ...
  const [showRealComplaintForm, setShowRealComplaintForm] = useState(false);
  ...
  const triggerScan = async (fileOrUrl: File | string, presetId?: string, title?: string) => {
    ...
    setShowRealComplaintForm(false);
...
Summary: Statutory mandatory declarations missing or malformed under Legal Metrology (Packaged Commodities) Rules, 2011.`}
            ...
            {/* Report a Complaint + Give Feedback - both attach automatically
                to this scan's inspection record, so the product never needs
                to be retyped. Available for every outcome, not just failures. */}
            {scanResult.inspectionId && (
              <div className="pt-2 space-y-3">
                <div className="flex flex-col sm:flex-row gap-2">
                  <button
                    onClick={() => {
                      setShowRealComplaintForm((v) => !v);
                      setShowFeedbackForm(false);
                    }}
                    className="flex-1 inline-flex items-center justify-center gap-2 rounded-lg border border-[#B42318]/40 bg-white px-4 py-2.5 text-xs font-bold text-[#B42318] hover:bg-[#FCEAE8] transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 inline-flex items-center justify-center gap-2 rounded-lg border border-[#B45309]/40 bg-white px-4 py-2.5 text-xs font-bold text-[#B45309] hover:bg-[#FDF3D8] transition-colors"
                  >
                ...
                {showRealComplaintForm && (
                  <ComplaintForm
                    inspectionId={scanResult.inspectionId}
                    productName={scanResult.productTitle}
                    onClose={() => setShowRealComplaintForm(false)}
                  />
                )}
                ...
                <ProductFeedbackPanel inspectionId={scanResult.inspectionId} refreshKey={feedbackRefreshKey} />
              </div>
            )}
          </div>
        )}
